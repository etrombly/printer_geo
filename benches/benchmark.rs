...
pub fn criterion_benchmark(c: &mut Criterion) {
    ...
    c.bench_function("par_iter bboxes", |b| {
        b.iter(|| {
            let bboxes: Vec<Line3d> = triangles.par_iter().map(|x| x.bbox()).collect();
        })
    });
    ...
    let bboxes3: Vec<Line3d> = tri_bbox_simd(&triangles);
    let bboxes4: Vec<Line3d> = tri_bbox_simd_par(&triangles);
    assert_eq!(bboxes[0], bboxes2[0]);

...
impl VkBuffer {
    pub fn new(vkstate: Rc<VulkanState>, size: u64) -> Self {
        ...
        let buffer = unsafe { vkstate.device.create_buffer(&buffer_create_info, None).unwrap() };
    ...
    pub fn get_buffer_memory_requirements(&self) -> vk::MemoryRequirements {
        unsafe { self.state.device.get_buffer_memory_requirements(self.buffer) }
    }
...
impl VkMem {
    ...
    pub fn map_buffer<T>(&self, data: &[T], buffer: &VkBuffer) {
        let pp_data: *mut T = unsafe {
            self.state
                .device
                .map_memory(self.mem, buffer.offset, buffer.size, vk::MemoryMapFlags::empty())
                .expect("[ERR] Could not map memory.") as *mut T
        };

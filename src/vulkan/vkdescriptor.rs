use crate::vulkan::{vkshader::VkShader, vkstate::VulkanState};
...
impl VkDescriptor {
    ...
    pub fn get_first_set(&self) -> Option<&vk::DescriptorSet> { self.set.first() }
}
...
impl VkWriteDescriptor {
    ...
    pub fn update_descriptors_sets(&self) {
        unsafe { self.state.device.update_descriptor_sets(&self.write_descriptors, &[]) };
    }
}

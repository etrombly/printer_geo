...
impl VkCmdPool {
    ...
    pub fn begin_cmd(&self, usage: vk::CommandBufferUsageFlags, cmd_buffer_index: usize) {
        ...
        unsafe {
            self.state
                .device
                .begin_command_buffer(self.cmd_buffers[cmd_buffer_index], &command_buffer_begin_info)
    ...
    pub fn bind_pipeline(&self, pipeline: vk::Pipeline, pipeline_type: vk::PipelineBindPoint, cmd_buffer_index: usize) {
        unsafe {
            self.state
                .device
                .cmd_bind_pipeline(self.cmd_buffers[cmd_buffer_index], pipeline_type, pipeline)
        };
    }
...
impl Drop for VkCmdPool {
    fn drop(&mut self) {
        unsafe {
            self.state.device.free_command_buffers(self.cmd_pool, &self.cmd_buffers);

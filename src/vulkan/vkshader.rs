...
impl VkShader {
    pub fn new(state: Rc<VulkanState>, shader_bytecode: Vec<u32>, entry_point: CString) -> Self {
        //let shader_bytecode = to_vec32(load_file(path).expect("[ERR] Could not load shader file."));

        let shader_module_create_info = vk::ShaderModuleCreateInfo::builder().code(&shader_bytecode).build();
    ...
    pub fn add_layout_binding(
        ...
        let descriptor_layout_binding_info = vk::DescriptorSetLayoutBinding::builder()
            ...
            .stage_flags(stage);
        self.layouts_bindings.push(descriptor_layout_binding_info.build());
    }
    ...
    pub fn create_pipeline_layout(&mut self) {
        ...
        let pipeline_layout_create_info = vk::PipelineLayoutCreateInfo::builder().set_layouts(&self.layout);
    ...
    fn drop(&mut self) {
        unsafe {
            ...
            for descriptor in self.layout.iter() {
                self.state.device.destroy_descriptor_set_layout(*descriptor, None);
            }
